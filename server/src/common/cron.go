...
func registerTask(name, spec string, f toolbox.TaskFunc) {
	t := toolbox.NewTask(name, spec, f)
	toolbox.AddTask(name, t)
}

// cron task init
func CronInit(fcInstance interface{}) (err error) {
	var cronTimer map[string]string
	//get cron task time from configuration
	err = json.Unmarshal([]byte(beego.AppConfig.String("cron::Timer")), &cronTimer)
	if err != nil {
		panic(err.Error())
	}

	//register cron task
	fc := reflect.ValueOf(fcInstance)
	fvZero := reflect.Value{}
	for k, v := range cronTimer {
		key, value := k, v
		fv := fc.MethodByName(key)
		if fv == fvZero { //while the function no found, just panic to gain attention of the administrator.
			panic(fmt.Sprintf("function cron.FunctionContainer.%s no found", key))
		}
		...
		LogFuncDebug("RegisterTask %s : %v", key, value)
		registerTask(key, value, f)
	}
