...
func New(baseURL, WalletBaseURL string) *API {
	api := &API{
		HttpClient: &http.Client{
...
func (api *API) GetTransaction(id string, blockId int) (out *TransactionRespV16, err error) {
	err = api.call("history", "get_transaction", M{"id": id, "block_num_hint": blockId}, &out)
	return
}
...
func (api *API) call(baseAPI string, endpoint string, body interface{}, out interface{}) error {
	...
	targetURL := ""
	if baseAPI == "wallet" {
		targetURL = fmt.Sprintf("%s/v1/%s/%s", api.WalletBaseURL, baseAPI, endpoint)
	} else {
