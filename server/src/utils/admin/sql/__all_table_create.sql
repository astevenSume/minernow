...
`sms_type` tinyint NOT NULL COMMENT 'sms_type',
PRIMARY KEY(`id`)
...
-- --------------------------------------------------
--  Table Structure for `models.MenuAccess`
-- --------------------------------------------------
...
PRIMARY KEY(`id`)
) ENGINE=InnoDB COMMENT='角色菜单权限表' DEFAULT CHARSET=utf8;

-- --------------------------------------------------
--  Table Structure for `models.MenuConf`
-- --------------------------------------------------
CREATE TABLE IF NOT EXISTS `menu_conf` (
`id` bigint unsigned NOT NULL AUTO_INCREMENT COMMENT 'id',
`pid` bigint unsigned NOT NULL COMMENT '父节点的id，一级菜单为0',
`level` int NOT NULL COMMENT '层级,1是一级菜单,2是二级菜单',
`name` varchar(100) NOT NULL COMMENT '菜单名称',
`path` varchar(100) NOT NULL COMMENT '菜单路径',
`icon` varchar(100) NOT NULL COMMENT '一级菜单图标',
`hide_in_menu` bool NOT NULL COMMENT '是否隐藏',
`component` varchar(100) NOT NULL COMMENT '组件',
`order_id` int unsigned NOT NULL COMMENT '排序id',
`ctime` bigint NOT NULL COMMENT '创建时间',
`utime` bigint NOT NULL COMMENT '修改时间',
PRIMARY KEY(`id`)
) ENGINE=InnoDB COMMENT='菜单配置' DEFAULT CHARSET=utf8;
...
-- --------------------------------------------------
--  Table Structure for `models.MonthDividendWhiteList`
-- --------------------------------------------------
CREATE TABLE IF NOT EXISTS `month_dividend_white_list` (
`id` int unsigned NOT NULL AUTO_INCREMENT COMMENT 'id',
`name` varchar(64) NOT NULL COMMENT 'name',
`dividend_ratio` int NOT NULL COMMENT 'dividend_ratio',
`ctime` bigint NOT NULL DEFAULT 0 COMMENT 'create time',
`utime` bigint NOT NULL DEFAULT 0 COMMENT 'update time',
PRIMARY KEY(`id`)
) ENGINE=InnoDB COMMENT='月分红白名单' DEFAULT CHARSET=utf8;
...
-- --------------------------------------------------
--  Table Structure for `models.ProfitThreshold`
-- --------------------------------------------------
CREATE TABLE IF NOT EXISTS `profit_threshold` (
`id` int unsigned NOT NULL AUTO_INCREMENT COMMENT 'id',
`threshold` bigint NOT NULL COMMENT 'threshold',
`admin_id` bigint unsigned NOT NULL COMMENT 'dividend_ratio',
`ctime` bigint NOT NULL DEFAULT 0 COMMENT 'create time',
`utime` bigint NOT NULL DEFAULT 0 COMMENT 'update time',
PRIMARY KEY(`id`)
) ENGINE=InnoDB COMMENT='分润阈值配置表' DEFAULT CHARSET=utf8;
...
CREATE UNIQUE INDEX `smstemplates_type` ON `smstemplates` (`type`);
